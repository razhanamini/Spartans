...
import {CheckProofRequestDto} from "../schemes/wallet.scheme";
import {tryParsePublicKey} from "../../wrappers/wallets-data";

const tonProofPrefix = 'ton-proof-item-v2/';
const tonConnectPrefix = 'ton-connect';
const allowedDomains = [
  'spartans-frontend-pied.vercel.app'
];
...
export class TonProofService {
  ...
  public async checkProof(payload: CheckProofRequestDto, getWalletPublicKey: (address: string) => Promise<Buffer | null>): Promise<boolean> {
    try {
      ...
      if (!publicKey) {
        console.log("Failed to obtain public key for address:", payload.address);
        return false;
      }
      ...
      if (!publicKey.equals(wantedPublicKey)) {
        console.log("Public key mismatch for address:", payload.address);
        return false;
      }
      ...
      if (!address.equals(wantedAddress)) {
        console.log("Address mismatch for public key:", payload.public_key);
        return false;
      }

      if (!allowedDomains.includes(payload.proof.domain.value)) {
        console.log("Domain not allowed:", payload.proof.domain.value);
        return false;
      }

      const now = Math.floor(Date.now() / 1000);
      if (now - validAuthTime > payload.proof.timestamp) {
        console.log("Proof timestamp is too old:", payload.proof.timestamp);
        return false;
      }
      ...
      console.log("Error during proof verification:", e);
      return false;
    }
  }
